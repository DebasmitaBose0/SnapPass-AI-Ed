  ...
  const handleCardClick = (session) => {
    if (session.hasOutput && session.processedUrl) {
      navigate('/print-preview', {
        state: { processedUrl: session.processedUrl, filename: session.filename },
      });
    }
  };
  ...
  return (
    <div className="history-page">
      ...
      <div className="history-page__count">
        {filtered.length === 0
          ? (search ? t.noResults || 'No matching sessions' : t.noHistory || 'No sessions yet')
      ...
      {filtered.length > 0 ? (
        ...
        <div className="history-page__empty">
          ...
          <div className="history-page__empty-title">
            {search ? t.noSearchResults || 'No results found' : t.noHistoryTitle || 'No session history'}
          </div>
          <div>
            {search
              ? (t.tryDifferentSearch || 'Try a different search term')
              : (t.startWithUpload || 'Upload and process a photo to get started')}
          </div>
        </div>
      )}

      {showConfirm && (
        <div className="history-page__confirm-overlay" onClick={() => setShowConfirm(false)}>
          <div className="history-page__confirm" onClick={(e) => e.stopPropagation()}>
            <div className="history-page__confirm-title">{t.confirmClearTitle || 'Clear all history?'}</div>
            <div className="history-page__confirm-text">
              {t.confirmClearText || 'This will permanently delete all saved sessions. This action cannot be undone.'}
            </div>
            <div className="history-page__confirm-actions">
              <button className="history-page__confirm-cancel" onClick={() => setShowConfirm(false)}>
                {t.cancel || 'Cancel'}
              </button>
              <button className="history-page__confirm-delete" onClick={handleClear}>
                {t.clearAll || 'Clear All'}
              </button>
            </div>
          </div>
