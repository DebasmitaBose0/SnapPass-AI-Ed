...
export const compressImage = (file, options = {}) => {
  const { maxWidth = 1920, maxHeight = 1920, quality = 0.85, resizeScale = 1.0, outputFormat } = options;
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        const preferredType = outputFormat
        ...
        canvas.toBlob((blob) => {
          ...
          resolve(new File([blob], outputName, { type: finalMime, lastModified: Date.now() }));
        }, finalMime, quality);
      };
    };
  });
